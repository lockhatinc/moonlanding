...
export async function patchCollaboratorRole(roleId, user, { role_type, role_description }) {
  const existingRole = get('collaborator_role', roleId);
  if (!existingRole) throw new AppError('Role not found', 'NOT_FOUND', HTTP.NOT_FOUND);
  if (!existingRole.is_active) throw new AppError('Cannot modify inactive role', 'VALIDATION_ERROR', HTTP.BAD_REQUEST);
  if (role_type && !canAssignRole(user.role, role_type)) throw new AppError(`User role ${user.role} cannot assign role ${role_type}`, 'FORBIDDEN', HTTP.FORBIDDEN);
  if (role_type && role_type !== existingRole.role_type) {
    return { reassigned: true, collaborator_id: existingRole.collaborator_id, old_role_type: existingRole.role_type };
  }
  const nowSeconds = Math.floor(Date.now() / 1000);
  return update('collaborator_role', roleId, {
...
export function deactivateCollaboratorRole(roleId, user) {
  const existingRole = get('collaborator_role', parseInt(roleId));
  if (!existingRole) throw new AppError('Role not found', 'NOT_FOUND', HTTP.NOT_FOUND);
  if (!existingRole.is_active) throw new AppError('Role already inactive', 'VALIDATION_ERROR', HTTP.BAD_REQUEST);
  if (!['partner', 'manager'].includes(user.role)) throw new AppError('Only partners and managers can revoke roles', 'FORBIDDEN', HTTP.FORBIDDEN);
  const nowSeconds = Math.floor(Date.now() / 1000);
  const updatedRole = update('collaborator_role', parseInt(roleId), {
    is_active: false, superseded_at: nowSeconds, updated_at: nowSeconds, updated_by: user.id
  });
  ...
  if (collaborator && collaborator.primary_role_id === existingRole.id) {
    update('collaborator', existingRole.collaborator_id, { primary_role_id: null, is_manager: false, updated_at: nowSeconds });
  }
  create('activity_log', {
    entity_type: 'collaborator_role', entity_id: existingRole.id, action: 'role_revoked',
    message: `Role ${existingRole.role_type} revoked by ${user.role}`,
    details: JSON.stringify({ collaborator_id: existingRole.collaborator_id, role_type: existingRole.role_type, revoked_by: user.id, revoked_at: nowSeconds })
  });
  return { updatedRole, collaborator, existingRole };
}
