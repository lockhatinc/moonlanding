...
export function renderChecklistDetails(user, checklist, items = []) {
  ...
  const itemRows = items.map(it => `<div class="flex items-center gap-3 p-2 border-b"><label class="flex items-center gap-3 flex-1"><input type="checkbox" class="checkbox" ${it.is_done ? 'checked' : ''} onchange="ckToggle('${it.id}',this.checked)" aria-label="${(it.name || it.title || 'item').replace(/"/g, '&quot;')}"/><span class="text-sm${it.is_done ? ' line-through text-gray-400' : ''}">${it.name || it.title || ''}</span></label><button class="btn btn-xs btn-ghost btn-error" onclick="ckDel('${it.id}')" aria-label="Delete ${(it.name || it.title || 'item').replace(/"/g, '&quot;')}">x</button></div>`).join('');
