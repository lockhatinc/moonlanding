...
export function renderRfiList(user, rfis = [], engagements = []) {
  ...
  const rows = rfis.map(r => {
    ...
    return `<tr class="rfi-row" data-status="${s}" style="border-bottom:1px solid #f0f0f0;cursor:pointer" onclick="window.location='/rfi/${r.id}'" onmouseover="this.style.background='#f9f9f9'" onmouseout="this.style.background=''">
      <td style="padding:10px 12px;font-size:0.82rem;font-weight:500">${r.name || r.title || r.id.slice(0,8)+'…'}</td>
      <td style="padding:10px 12px"><a href="/engagement/${r.engagement_id}" onclick="event.stopPropagation()" style="color:#1565c0;text-decoration:none;font-size:0.82rem">${eng.name || eng.client_name || r.engagement_id?.slice(0,8)+'…' || '—'}</a></td>
