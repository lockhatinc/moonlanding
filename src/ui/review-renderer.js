...
export function renderReviewListTabbed(user, reviews) {
  ...
  const emptyState = `<tr><td colspan="6" style="text-align:center;padding:60px 20px">
    <div style="font-size:2.5rem;opacity:0.2;margin-bottom:12px">📝</div>
    <div style="font-weight:600;color:#333;margin-bottom:6px">No reviews yet</div>
    <div style="color:#aaa;font-size:0.82rem">Start a new review to get started.</div>
  </td></tr>`;
  const table = `<div style="background:#fff;border-radius:8px;box-shadow:0 1px 4px rgba(0,0,0,0.1)">
    <div style="padding:10px 12px;border-bottom:1px solid #f0f0f0">
      <span id="review-count" style="font-size:0.78rem;color:#888">Showing <strong>${reviews.length}</strong> reviews</span>
    </div>
    <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:0.82rem"><thead><tr style="background:#fafafa;border-bottom:2px solid #e0e0e0">${headers}</tr></thead><tbody id="review-tbody">${rows || emptyState}</tbody></table></div>
  </div>`;
  const content = `<div class="page-header" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:10px"><h1 style="font-size:1.25rem;font-weight:700;margin:0;color:#04141f">Reviews</h1><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">${reviewSearchField()}${hideEmptyReviewsToggle()}${createBtn}</div></div>${tabBar}${table}${reviewContextMenu()}`;
  const tabScript = `window.switchTab=(key)=>{document.querySelectorAll('.review-tab').forEach(t=>{var isA=t.dataset.tab===key;t.classList.toggle('review-tab-active',isA);t.style.borderBottomColor=isA?'#1976d2':'transparent';t.style.color=isA?'#1976d2':'#666'});const rows=document.querySelectorAll('.review-row');var vis=0,tot=0;rows.forEach(r=>{const s=r.dataset.status,a=r.dataset.archived,p=r.dataset.priority;let show=true;if(key==='active')show=s==='active'||s==='open'||s==='in_progress';else if(key==='priority')show=p==='1';else if(key==='history')show=s==='completed'||s==='closed';else if(key==='archive')show=s==='archived'||a==='1';r.style.display=show?'':'none';tot++;if(show)vis++});var rc=document.getElementById('review-count');if(rc)rc.innerHTML='Showing <strong>'+vis+'</strong> of '+tot+' reviews'}`;
