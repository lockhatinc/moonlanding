...
export function renderLogin(error = null, hasGoogleAuth = false) {
  ...
  const googleBtn = hasGoogleAuth ? `
    <a href="/api/auth/google" style="display:flex;align-items:center;justify-content:center;gap:10px;width:100%;padding:11px 12px;border:1px solid #ddd;border-radius:6px;background:#fff;color:#333;font-size:0.88rem;font-weight:500;text-decoration:none;margin-bottom:16px;transition:all 0.15s;outline:none;cursor:pointer" onmouseover="this.style.background='#f8f9fa';this.style.borderColor='#bbb'" onmouseout="this.style.background='#fff';this.style.borderColor='#ddd'" onfocus="this.style.outline='2px solid #90CAF9';this.style.outlineOffset='1px'" onblur="this.style.outline='none'">
  ...
  const body = `
  <div style="min-height:100vh;background:#f0f2f5;display:flex;align-items:center;justify-content:center;padding:16px">
    <div style="width:100%;max-width:420px">
      <div style="text-align:center;margin-bottom:28px">
        <div style="display:inline-flex;align-items:center;gap:10px;margin-bottom:12px">
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#1976d2" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M12 1v6m0 6v6M4.22 4.22l4.24 4.24m4.24 4.24l4.24 4.24M1 12h6m6 0h6M4.22 19.78l4.24-4.24m4.24-4.24l4.24-4.24"/></svg>
          <span style="font-size:1.4rem;font-weight:800;color:#1976d2;letter-spacing:1px">MOONLANDING</span>
        </div>
        <p style="color:#666;font-size:0.9rem;margin:0">Sign in to your account</p>
      </div>
      <div style="background:#fff;border-radius:12px;box-shadow:0 2px 16px rgba(0,0,0,0.1);padding:24px">
        ${errHtml}
        ${googleBtn}
        <form id="login-form" aria-label="Sign in">
          ...
          <button type="submit" id="login-btn" style="width:100%;padding:11px;background:#1976d2;color:#fff;border:none;border-radius:6px;font-size:0.92rem;font-weight:600;cursor:pointer;transition:all 0.15s;outline:none"
            onmouseover="this.style.background='#0d47a1'" onmouseout="this.style.background='#1976d2'" onfocus="this.style.outline='2px solid #64B5F6';this.style.outlineOffset='2px'" onblur="this.style.outline='none'">
            Sign In
          </button>
        </form>
      </div>
...
export function renderPasswordReset() {
  const body = `<div class="center-screen"><div class="card w-96 bg-white shadow-lg"><div class="card-clean-body">
    ...
    <div id="reset-success" style="display:none;background:#d4edda;border:1px solid #c3e6cb;color:#155724;padding:12px;border-radius:6px;margin-bottom:16px;font-size:0.85rem">If an account exists with that email, a password reset link has been sent.</div>
    <form id="reset-form" aria-label="Password reset request"><div class="form-group"><label class="form-label" for="email">Email</label><input type="email" name="email" id="email" class="input input-bordered w-full" placeholder="Enter your email" required style="width:100%;padding:10px 12px;border:1px solid #ddd;border-radius:6px;font-size:0.88rem;box-sizing:border-box;transition:border-color 0.15s" onfocus="this.style.borderColor='#1976d2';this.style.outline='2px solid #90CAF9';this.style.outlineOffset='1px'" onblur="this.style.borderColor='#ddd';this.style.outline='none'"/></div>
    <button type="submit" id="reset-btn" class="btn btn-primary w-full mt-6" style="width:100%;padding:11px;background:#1976d2;color:#fff;border:none;border-radius:6px;font-size:0.92rem;font-weight:600;cursor:pointer;transition:all 0.15s;outline:none;margin-top:16px" onmouseover="this.style.background='#0d47a1'" onmouseout="this.style.background='#1976d2'" onfocus="this.style.outline='2px solid #64B5F6';this.style.outlineOffset='2px'" onblur="this.style.outline='none'">Send Reset Link</button></form>
    <div class="text-center mt-4"><a href="/login" class="text-sm" style="color:#1976d2;text-decoration:none" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">Back to login</a></div></div></div></div>`
...
export function renderPasswordResetConfirm(token) {
  const body = `<div class="center-screen"><div class="card w-96 bg-white shadow-lg"><div class="card-clean-body">
    ...
    <div id="confirm-success" style="display:none;background:#d4edda;border:1px solid #c3e6cb;color:#155724;padding:12px;border-radius:6px;margin-bottom:16px;font-size:0.85rem">Password updated successfully. <a href="/login" style="color:#155724;text-decoration:underline">Sign in</a></div>
    <form id="confirm-form" aria-label="Set new password"><input type="hidden" name="token" value="${token}"/>
    <div class="form-group"><label class="form-label" for="password">New Password</label><input type="password" name="password" id="password" class="input input-bordered w-full" placeholder="Enter new password" minlength="8" required style="width:100%;padding:10px 12px;border:1px solid #ddd;border-radius:6px;font-size:0.88rem;box-sizing:border-box;transition:border-color 0.15s" onfocus="this.style.borderColor='#1976d2';this.style.outline='2px solid #90CAF9';this.style.outlineOffset='1px'" onblur="this.style.borderColor='#ddd';this.style.outline='none'"/></div>
    <div class="form-group mt-4"><label class="form-label" for="confirm_password">Confirm Password</label><input type="password" name="confirm_password" id="confirm_password" class="input input-bordered w-full" placeholder="Confirm new password" minlength="8" required style="width:100%;padding:10px 12px;border:1px solid #ddd;border-radius:6px;font-size:0.88rem;box-sizing:border-box;transition:border-color 0.15s" onfocus="this.style.borderColor='#1976d2';this.style.outline='2px solid #90CAF9';this.style.outlineOffset='1px'" onblur="this.style.borderColor='#ddd';this.style.outline='none'"/></div>
    <button type="submit" id="confirm-btn" class="btn btn-primary w-full mt-6" style="width:100%;padding:11px;background:#1976d2;color:#fff;border:none;border-radius:6px;font-size:0.92rem;font-weight:600;cursor:pointer;transition:all 0.15s;outline:none;margin-top:16px" onmouseover="this.style.background='#0d47a1'" onmouseout="this.style.background='#1976d2'" onfocus="this.style.outline='2px solid #64B5F6';this.style.outlineOffset='2px'" onblur="this.style.outline='none'">Update Password</button></form>
    <div class="text-center mt-4"><a href="/login" class="text-sm" style="color:#1976d2;text-decoration:none" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">Back to login</a></div></div></div></div>`
