...
async function loadChecklist(){
  ...
  try{
    ...
    el.innerHTML=items.length?items.map(function(item){return'<div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid #f0f0f0"><input type="checkbox" data-id="'+item.id+'"'+(item.completed?' checked':'')+' onchange="toggleCheckItem(this.dataset.id,this.checked)" style="width:18px;height:18px;cursor:pointer;accent-color:#1976d2"/><span style="font-size:0.85rem;'+(item.completed?'text-decoration:line-through;color:#aaa':'')+'">'+(item.name||'Item')+'</span></div>'}).join(''):'<div style="text-align:center;color:#aaa;font-size:0.82rem;padding:32px">No checklist items yet</div>'}
