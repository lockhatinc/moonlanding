...
import { queueEmail, sendQueuedEmails, generateChecklistPdf } from '../engine/email-templates.js';
import { ENGAGEMENT_STAGE, ENGAGEMENT_STATUS, RFI_STATUS, canTransitionStage } from './status-helpers.js';
...
const recreateEngagements = async (interval) => {
  const { recreateEngagement } = await import('../engine/recreation.js');
...
const scheduledJobs = {
  daily_backup: { schedule: '0 2 * * *', description: 'Export database to backup', run: async () => { const { exportDatabase } = await import('../engine/backup.js'); await exportDatabase(); } },
  daily_user_sync: {
    schedule: '0 3 * * *',
