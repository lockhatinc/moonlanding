import { Group, Box, Title, ActionIcon, Button, Stack } from '@mantine/core';
...
export function DetailHeader({ spec, data, canEdit, canDelete, onDelete, icon: Icon }) {
  ...
  const breadcrumbItems = [
    { label: spec.labelPlural, href: `/${spec.name}`, icon: spec.icon },
  ...
  return (
    <Stack gap="md">
      ...
      <Group justify="space-between">
        <Group>
          <ActionIcon variant="subtle" onClick={() => router.push(`/${spec.name}`)} aria-label="Go back"><ACTION_ICONS.back size={18} /></ActionIcon>
          <Icon size={24} />
          <Box>
