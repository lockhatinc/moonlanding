...
export const specs = {
  ...
  engagement: {
    ...
    fields: {
      ...
      // Progress tracking
      progress: { type: 'decimal', label: 'Progress', default: 0 },
      client_progress: { type: 'decimal', label: 'Client Progress', default: 0 },
      fee: { type: 'decimal', label: 'Fee' },
      // Engagement letter
      letter_status: { type: 'enum', label: 'Letter Status', options: 'letter_statuses', default: 'pending' },
      letter_auditor_status: { type: 'enum', label: 'Auditor Letter Status', options: 'auditor_letter_statuses', default: 'pending' },
      letter_drive_id: { type: 'text', label: 'Letter File', hidden: true },
      // Post-RFI
      post_rfi_client_status: { type: 'enum', label: 'Post-RFI Client', options: 'post_rfi_statuses', default: 'pending' },
      ...
      recreate_with_attachments: { type: 'bool', label: 'Recreate With Attachments', default: false },
      // Assignments
      ...
      // Feedback
      feedback: { type: 'json', label: 'Client Feedback' },
    ...
    options: {
      ...
      stages: [
        ...
        { value: 2, label: 'Commencement' },
        { value: 3, label: 'Team Execution' },
        { value: 4, label: 'Partner Review' },
        { value: 5, label: 'Finalization' },
        { value: 6, label: 'Close Out' },
      ],
      letter_statuses: [
        { value: 'pending', label: 'Pending', color: 'yellow' },
    ...
    form: {
      sections: [
        ...
        { label: 'Dates', fields: ['commencement_date', 'deadline', 'completion_date'] },
        { label: 'Team', fields: ['team_id'] },
        { label: 'Settings', fields: ['repeat_interval', 'recreate_with_attachments', 'clerks_can_approve'] },
      ],
    },
    list: { defaultSort: { field: 'created_at', dir: 'desc' }, filters: ['status', 'year', 'team_id', 'stage'] },
  },
  ...
  review: {
    ...
    fields: {
      ...
      stage: { type: 'enum', label: 'Stage', options: 'stages', default: 'draft' },
      ...
      // Highlights stored in review (MWR style)
      ...
      // Tender
      is_tender: { type: 'bool', label: 'Is Tender', default: false },
      tender_details: { type: 'json', label: 'Tender Details' },
      // ML
      has_email_checklist: { type: 'bool', label: 'Has Email Checklist', default: false },
      // Friday integration
      friday_link: { type: 'text', label: 'Friday Engagement ID' },
      // Manager
      first_manager: { type: 'ref', ref: 'user', label: 'First Manager' },
      // Audit
    ...
    access: {
      ...
      add_collaborator: ['partner', 'manager'],
      add_flag: ['partner'],
      add_tag: ['partner', 'manager'],
    },
  },
  ...
  rfi: {
    ...
    fields: {
      ...
      section_id: { type: 'ref', ref: 'engagement_section', label: 'Section' },
      ...
      // Status tracking
      status: { type: 'int', label: 'Status', default: 0 }, // 0=waiting, 1=completed
      rfi_status: { type: 'enum', label: 'RFI Status', options: 'rfi_statuses', default: 'pending' },
      ...
      // Dates
      date_requested: { type: 'date', label: 'Date Requested' },
      date_resolved: { type: 'date', label: 'Date Resolved' },
      deadline: { type: 'date', label: 'Deadline', list: true },
      deadline_date: { type: 'date', label: 'Deadline Date' },
      days_outstanding: { type: 'int', label: 'Days Outstanding', default: 0 },
      // Responses and files (stored as JSON arrays)
      responses: { type: 'json', label: 'Responses' },
      files: { type: 'json', label: 'Files' },
      files_count: { type: 'int', label: 'Files Count', default: 0 },
      response_count: { type: 'int', label: 'Response Count', default: 0 },
      // Assignments
      users: { type: 'json', label: 'Assigned Users' },
      // Flags
      flag: { type: 'bool', label: 'Flagged', default: false },
  ...
  highlight: {
    ...
    fields: {
      ...
      // Position data for PDF rendering
      position: { type: 'json', label: 'Position', required: true, hidden: true },
      // Content
      content: { type: 'json', label: 'Selected Content' }, // { text?, image? }
      comment: { type: 'json', label: 'Comment' }, // { text, emoji }
      ...
      // Color (partner highlights are red)
      color: { type: 'text', label: 'Color', default: '#B0B0B0' },
      is_partner_highlight: { type: 'bool', label: 'Partner Highlight', default: false },
      // Responses
      responses: { type: 'json', label: 'Responses' },
      // Audit
  ...
  user: {
    ...
    fields: {
      ...
      uid: { type: 'text', label: 'Firebase UID', hidden: true },
      ...
      role_id: { type: 'ref', ref: 'user_group', label: 'Role Group' },
      status: { type: 'enum', label: 'Status', options: 'statuses', list: true, default: 'active' },
      auth_provider: { type: 'enum', label: 'Auth Provider', options: 'auth_providers', default: 'email' },
      // Client user fields
      clients: { type: 'json', label: 'Client Access' }, // [{client_id, role: admin|user, status}]
      // MWR fields
  ...
  // === COLLABORATOR (temporary access) ===
  collaborator: {
    name: 'collaborator', label: 'Collaborator', labelPlural: 'Collaborators', icon: 'UserPlus', embedded: true,
    fields: {
      id: { type: 'id' },
      review_id: { type: 'ref', ref: 'review', required: true },
      user_id: { type: 'ref', ref: 'user', required: true, list: true, display: 'user.name' },
      is_temporary: { type: 'bool', label: 'Temporary', default: false },
      expires_at: { type: 'timestamp', label: 'Expires At' },
      added_by: { type: 'ref', ref: 'user', auto: 'user' },
      created_at: { type: 'timestamp', auto: 'now', readOnly: true },
    },
    access: {
  ...
  email: {
    ...
    fields: {
      ...
      allocated_to_client: { type: 'ref', ref: 'client' },
      allocated_to_engagement: { type: 'ref', ref: 'engagement' },
      allocated_to_rfi: { type: 'ref', ref: 'rfi' },
  ...
  removed_highlight: {
    name: 'removed_highlight', label: 'Archived Query', labelPlural: 'Archived Queries', icon: 'Archive', embedded: true,
    fields: {
      id: { type: 'id' },
      review_id: { type: 'ref', ref: 'review', required: true },
      original_highlight: { type: 'json', label: 'Original Data', required: true },
